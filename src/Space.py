...
class Graph:
    ...
    def is_start(self, node:Node) -> bool:
        '''
        kiểm tra 'node' có phải là nguồn hay không
        '''
    ...
    def get_heuristic(self, node:Node):
        '''
        trả về giá trị heuristic của node
        '''
        return math.sqrt((node.x-self.goal.x)*(node.x-self.goal.x)+(node.y-self.goal.y)*(node.y-self.goal.y))
